...
def return_tokenize_fn(tokenizer, user_tokens, markers):
    ...
    def tokenize_w_label(idx, content, claim, premise):
        inputs = tokenizer(content.numpy().decode('utf-8'), truncation=True, return_tensors='tf', return_offsets_mapping=True)
        enc_tokens = list(inputs['input_ids'].numpy()[0])
        labels = [0 for _ in enc_tokens]
        marker_features = utils.get_marker_features(content.numpy().decode('utf-8'), markers, list(inputs['offset_mapping'].numpy())[0])
        ...
        return (idx, 
                    ...
                    tf.convert_to_tensor(marker_features, dtype=tf.int32),
                    labels)
...
def load_data_w_label(dir_name, tokenizer, user_tokens):
    ...
    dataset = dataset.map(lambda x,y,z,w:tf.py_function(return_tokenize_fn(tokenizer, user_tokens, markers),
                                                  inp=[x,y,z,w],
                                                  Tout=(tf.string, tf.int32, tf.int32, tf.int32, tf.int32, tf.int32))) #, tf.float32)), 
