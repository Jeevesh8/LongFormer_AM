...
config = {
    "max_rel_length": 2
    + MAX_TOKEN_DIST
    - MIN_TOKEN_DIST,  # 0 for no token to refer. 1 for title. Rest for various distances.
    "dist_to_label": {
        i: j + 2 for j, i in enumerate(range(MIN_TOKEN_DIST, MAX_TOKEN_DIST))
    },
    "relations": [
        "partial_attack",
        "agreement",
...
config["pad_for"] = {
    ...
    "comp_type_labels": config["arg_components"][
        "other"
    ],  # len(config['arg_components']),
