...
def get_label_component_list(filename):
    ...
    for k in begin_positions.keys():
        ...
        component_type = (
            config["arg_components"]["B-C"]
            if comp_types[k] == "claim"
            else config["arg_components"]["B-P"]
        )
        refers_to = [
            elem if elem == "None" else begin_positions[elem]
            for elem in str(ref_n_rel_type[k][0]).split("_")
        ]
...
def get_begin_from_refers(relative_dist_label, prev_comment_begin_position):
    ...
    if begin_idx_of_related_component > 0:
        print(
            "The begin index of related component is coming out to be negative in the predictions!! Previous comment begin position: ",
            prev_comment_begin_position,
            " & The relative distance predicted: ",
            relative_dist,
        )
    return begin_idx_of_related_component


def get_pred_component_list(
    seq_length, filename, viterbi_seq, refers_preds, relation_type_preds
):
    ...
    prev_comment_begin_positions, begin_positions = get_prev_comment_begin_positions(
        filename
    )
    j = 0
    ...
    if viterbi_seq[j] == config["arg_components"]["I-C"]:
        viterbi_seq[j] = (
            config["arg_components"]["B-C"]
            if viterbi_seq[j + 1] == config["arg_components"]["I-C"]
            else config["arg_components"]["other"]
        )
    if viterbi_seq[j] == config["arg_components"]["I-P"]:
        viterbi_seq[j] = (
            config["arg_components"]["B-P"]
            if viterbi_seq[j + 1] == config["arg_components"]["I-P"]
            else config["arg_components"]["other"]
        )
    ...
    while j < seq_length:
        if viterbi_seq[j] == config["arg_components"]["B-C"]:
            ...
            prev_comment_begin_position = get_prev_comment_begin_position(
                begin, prev_comment_begin_positions
            )
            refers_to = (
                begin_positions["title"]
                if refers_preds[begin] == 1
                else get_begin_from_refers(
                    refers_preds[begin], prev_comment_begin_position
                )
            )
            rel_type = config["relations"][relation_type_preds[begin]]

            predicted_components.append(
                (begin, end, component_type, refers_to, rel_type)
            )
            ...
            while (
                j < seq_length and viterbi_seq[j] == config["arg_components"]["other"]
            ):
                j += 1
...
def change_input_dtypes(func):
    def new_func(filename, seq_length, viterbi_seq, refers_preds, relation_type_preds):
        for i in range(len(filename)):
            func(
                filename[i].numpy().decode("utf-8"),
                int(seq_length[i].numpy()),
                viterbi_seq[i].numpy().tolist(),
                refers_preds[i].numpy().tolist(),
                relation_type_preds[i].numpy().tolist(),
            )
...
def single_sample_eval(
    ...
    """
    print(
        "Args: ", filename, seq_length, viterbi_seq, refers_preds, relation_type_preds
    )
    label_list = get_label_component_list(filename)
    preds_list = get_pred_component_list(
        seq_length, filename, viterbi_seq, refers_preds, relation_type_preds
    )
    ...
    for j, elem in enumerate(component_preds_list):
        if elem[2] == config["arg_components"]["B-C"]:
            ...
            raise ValueError(
                "Can't find component type ",
                elem[2],
                " in argumentative component types:  [",
                config["arg_components"]["B-C"],
                ", ",
                config["arg_components"]["B-P"],
            )
    ...
    print(
        "Total links between components correct-ly predicted. Including None links: ",
        all_links,
    )
    print("Correct links between componenets correctly predicted: ", valid_links)
    print(
        "Correct Links between correct components with correct types: ", valid_rel_types
    )
