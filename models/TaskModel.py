...
def get_transition(num_classes, max_val=0.1, min_val=-0.1, is_padded=False):
    ...
    T[0, [2, 4]] = [-10000.0, -10000.0]
    T[1, [0, 1, 3, 4]] = T[3, [0, 1, 2, 3]] = [-10000.0, -10000.0, -10000.0, -10000.0]
    T[2, [4]] = T[4, [2]] = [-10000.0]
    if is_padded:
        T[5, :] = [-1e-5 for _ in range(num_classes)]
        T[[1, 3], 5] = [-10000.0, -10000.0]
        T[5, 5] = 0
...
class TaskModel(tf.keras.models.Model):
    def __init__(
        ...
        self.crf_layer = CRF(
            get_transition(
                num_classes, max_val=max_trans, min_val=min_trans, is_padded=is_padded
            )
        )
    ...
    def call(self, inputs, training=True):
        ...
        crf_predictions = self.crf_layer(
            logits, mask=inputs["attention_mask"], training=training
        )
        ...
        if not training:
            return tuple(
                (
                    *crf_predictions,
                    self.relation_type_predictor(encoded_seq),
                    self.refers_predictor(encoded_seq),
                )
            )
    ...
    def compute_batch_sample_weight(self, labels, pad_mask, max_possible_length):
        counts = tf.reduce_sum(
            tf.cast(
                tf.equal(
                    tf.expand_dims(tf.range(0, max_possible_length), -1),
                    tf.reshape(labels, [-1]),
                ),
                dtype=tf.int32,
            ),
    ...
    def get_cross_entropy(self, logits, labels, pad_mask, max_possible_length):
        sample_weight = self.compute_batch_sample_weight(
            labels, pad_mask, max_possible_length=max_possible_length
        )
        cc_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels, logits)
        n_samples = tf.reduce_sum(sample_weight)
        return (
            tf.reduce_sum(cc_loss * sample_weight) / n_samples
            if n_samples != 0
            else tf.convert_to_tensor(0.0)
        )
    ...
    def compute_loss(self, x, y):
        ...
        crf_loss = -crf_log_likelihood(
            potentials, comp_type_labels, sequence_length, chain_kernel
        )[0]
        comp_type_cc_loss = self.get_cross_entropy(
            logits, comp_type_labels, x["attention_mask"], len(config["arg_components"])
        )
        relation_type_cc_loss = self.get_cross_entropy(
            relation_type_logits,
            relation_type_labels,
            x["attention_mask"],
            len(config["relations"]),
        )
        refers_cc_losses = tf.map_fn(
            lambda labels: self.get_cross_entropy(
                refers_logits, labels, x["attention_mask"], config["max_rel_length"]
            ),
            tf.transpose(refers_labels, perm=(2, 0, 1)),
    ...
    def infer_step(self, x):
        viterbi_seqs, seq_lens, relation_type_logits, refers_logits = self(
            x, training=False
        )
