...
def _create_min_max_boundaries(max_length, min_boundary=256, boundary_scale=1.1):
...
def get_model(max_pos, attention_window, offset_out=True):
    ...
    new_pos_embed[
        ...
        offset : offset + min(4096, max_pos)
    ]
    ...
    while k + step <= max_pos:
        new_pos_embed[
            k : (k + step)
        ] = model.longformer.embeddings.position_embeddings.weight[offset:]
        k += step

    new_pos_embed[k:] = model.longformer.embeddings.position_embeddings.weight[
        offset : (max_pos - k) + offset
    ]
    ...
    model.longformer.embeddings.position_ids.data = torch.tensor(
        [i for i in range(max_pos + offset if offset_out else max_pos)]
    ).reshape(1, -1)
