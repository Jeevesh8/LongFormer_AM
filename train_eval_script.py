...
task_optimizer, _ = create_optimizer(
    init_lr=0.00005, num_train_steps=config["n_iters"], num_warmup_steps=80
)
...
def get_datasets(file_list):
    ...
    return (
        ...
        .cahce()
    )  # .repeat(1 if not repeat else int((config['n_iters']*config['batch_size'])/len(file_list))+1).cache()
