...
with open("./train_test_split.txt") as f:
    lines = f.read()
    train_files = [
        elem.split("/")[-1][:-4].split("_")
        for elem in list(filter(None, lines.split("\n")[0].split("\t")))
    ]
    test_files = [
        elem.split("/")[-1][:-4].split("_")
        for elem in list(filter(None, lines.split("\n")[1].split("\t")))[10:]
    ...
    train_files = [
        "./AmpersandData/change-my-view-modes/v2.0/" + elem[1] + "/" + elem[0] + ".xml"
...
for inp in test_dataset:
    ...
    for p, l, length in zip(
        list(viterbi_seqs.numpy()), list(inp[2].numpy()), list(seq_lens.numpy())
    ):
