...
def add_elem(
    ...
    claims_lis: List[str],
    premise_lis: List[str],
    to_claim: Optional[bool] = None,
):
...
def update_claims_premises(
    ...
    if str(elem).strip().startswith("<claim"):
        ...
        try:
            ...
            claim_ref, claim_rel = str(difference), str(
        ...
        for part in parsed_claim.find("claim").contents:
            if (
                not str(part).startswith("<claim")
                and not str(part).startswith("<premise")
                and not part == parsed_claim.find("claim").contents[0]
            ):
                claim_ref = claim_id
                claim_id += "Ć"  # c for child/continue
                claim_rel = "cont"
            update_claims_premises(
                part, claims, premises, True, claim_id, claim_ref, claim_rel
            )
        ...
        try:
            ...
            difference = ref_id  # elem_nos[ref_id]-elem_nos[claim_id]
            premise_ref, premise_rel = str(difference), str(
                parsed_premise.find("premise")["rel"]
            )
        except:
        ...
        for part in parsed_premise.find("premise").contents:
            if (
                not str(part).startswith("<claim")
                and not str(part).startswith("<premise")
                and not part == parsed_premise.find("premise").contents[0]
            ):
                premise_ref = premise_id
                premise_id += "Ć"  # c for child/continue
                premise_rel = "cont"
            update_claims_premises(
                part, claims, premises, False, premise_id, premise_ref, premise_rel
            )
        ...
        try:
            ...
            raise AssertionError(
                "Claim : ", claim, " not found in existing claims: ", claims
            )
        claims[idx] = (
            parent_id + "\t" + parent_ref + ":" + parent_rel + "\t" + claims[idx]
        )
        ...
        try:
            ...
            raise AssertionError(
                "Premise : ", premise, " not found in existing claims: ", premises
            )
        premises[idx] = (
            parent_id + "\t" + parent_ref + ":" + parent_rel + "\t" + premises[idx]
        )


def add_distance_relation(
    parsed_xml: bs4.BeautifulSoup, claims: List[str], premises: List[str]
):
    ...
    try:
        ...
        raise AssertionError(
            "Title claim not found ! In thread with claims and premises: ",
            claims,
            premises,
        )
        ...
        for f in os.listdir(root):
            ...
            if os.path.isfile(filename) and f.endswith(".xml"):
                ...
                parsed_xml = BeautifulSoup(
                    clean_component(str(BeautifulSoup(xml_str, "lxml"))), "xml"
                )
                claims, premises = get_claims_premise(parsed_xml)

                with open(
                    os.path.join("AmpersandData/Threads", f[:-4] + "_" + t + ".claim"),
                    "w",
                ) as g:
                    for claim in claims:
                        g.write(claim + "\n")
                ...
                with open(
                    os.path.join(
                        "AmpersandData/Threads", f[:-4] + "_" + t + ".premise"
                    ),
                    "w",
                ) as g:
                ...
                with open(
                    os.path.join(
                        "AmpersandData/ModifiedThreads", f[:-4] + "_" + t + ".claim"
                    ),
                    "w",
                ) as g:
                    for line in claims:
                ...
                with open(
                    os.path.join(
                        "AmpersandData/ModifiedThreads", f[:-4] + "_" + t + ".premise"
                    ),
                    "w",
                ) as g:
