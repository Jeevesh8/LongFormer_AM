...
if __name__ == "__main__":
                        ...
                        if elem.startswith("<claim") or elem.startswith("<premise"):
                            ...
                            try:
                                if len(parsed_component.find("claim").contents) > 1:
                                    ...
                                    print(
                                        "Content Nested inside Claim : ",
                                        parsed_component.find("claim").contents,
                                    )
                            except:
                                if len(parsed_component.find("premise").contents) > 1:
                                    nested_claim_premise += 1
                                    print(
                                        "Content Nested inside Premise : ",
                                        parsed_component.find("premise").contents,
                                    )
                            try:
                                ...
                                try:
                                    ref_ids = str(
                                        parsed_component.find("premise")["ref"]
                                    )
                                    component_id = str(
                                        parsed_component.find("premise")["id"]
                                    )
                                except:
                                    continue
                            ...
                            for ref_id in ref_ids.split("_"):
                                if (
                                    ref_id not in cur_claim_premise
                                    and ref_id not in prev_claim_premise
                                ):
                                    two_post_farther += 1
                                    far_away_ids.append(
                                        (f[:-4] + "_" + t, ref_id, component_id)
                                    )
    ...
    print(
        "Relations inside current reply or in previous post/title[Excluding nested components] : ",
        closer_than_two_post,
    )
    print(
        "Relations not found inside current reply and in previous post/title[Excluding nested components] : ",
        two_post_farther,
    )
    ...
    user_token_indices = [
        tokenizer.encode("[USER0]")[1:-1][0],
        tokenizer.encode("[USER1]")[1:-1][0],
    ]
    ...
    for f in os.listdir(root):
        ...
        if (
            ...
            content = "\n".join(
                [" ".join(elem.split()) for elem in content.split("\n")]
            )
                ...
                with open(os.path.join(cnp_root, f[:-4] + component_type)) as g:
                    for elem in g.read().split("\n"):
                        ...
                        if indices is None or (
                            alt_indices is not None and alt_indices[0] < indices[0]
                        ):
                            indices = alt_indices

                        assert indices is not None, (
                            str(component) + " in " + filename
                        )  # "Unable to find claim : "+ str(tokenizer.convert_ids_to_tokens(tokenized_component)) + " in " + str(tokenizer.convert_ids_to_tokens(input_ids)) #str(component) + " in "+ content
                        ...
                        prev_comment_begin_position[component_id] = find_last_to_last(
                            input_ids[: indices[0]], user_token_indices
                        )
                    ...
                    for elem in g.read().split("\n"):
                        ...
                        for ref in ref.split("_"):
                            if ref != "None" and ref != "title":
                                distances.append(
                                    begin_positions[ref] - begin_positions[component_id]
                                )
                                dist_relative_to_prev_comment_beginning.append(
                                    begin_positions[ref]
                                    - prev_comment_begin_position[component_id]
                                )
                                if dist_relative_to_prev_comment_beginning[-1] < 0:
                                    print(
                                        "Very far away referenced: ",
                                        ref,
                                        " from ",
                                        component_id,
                                    )
    ...
    print(
        "Distances relative to beginning of previous comment(excluding relations to title) : "
    )
    print(
        max(dist_relative_to_prev_comment_beginning),
        min(dist_relative_to_prev_comment_beginning),
        dist_relative_to_prev_comment_beginning,
    )
